...
public class MemoryAccess implements IMemoryAccess {
 ...
 private int bufferSize;
   ...
   /**
    * Class constructor takes the processor memory and the pre-MEM buffer size as parameters
    * 
   */
   ...
   * Creates a skeleton MemoryAccess with only the post-MEM buffer set
  */
   ...
   */
   public void processClockCycle()
   {
     ...
     if (stallCycles == 0) {
       ...
       switch (currentInstruction.opName) {
       	case "lw":
       		addToPostMEM(memList.getValueAt(currentInstruction.rs + currentInstruction.immediate));
       		break;
       	case "sw":
       		memList.setValueAt((currentInstruction.rs + currentInstruction.immediate),(Integer)currentInstruction.rt);
       		break;
       	case "lb":
       		addToPostMEM(memList.getValueAt(currentInstruction.rs + currentInstruction.immediate));
       		break;
   ...
   public int getPreMEMimmVal(int index){return this.preMEMBuffer[index].immediate; }
   ...
   private class PreMEMBufferEntry
   {
     ...
     private int rs;         //for loads, the register number where the data value will be loaded to
                             //for stores, the actual value to be stored (passed from the issue unit)
