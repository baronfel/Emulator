   ...
   */
   public void processClockCycle()
   {
     ...
     if (stallCycles == 0) {
       ...
       if (tmpStr == "lw") {
         double memValue = memList.getValueAt(currentInstruction.rt + currentInstruction.immediate);
         //add the result to the post MEM buffer
         addToPostMEM(memValue);
       }
       else if (tmpStr == "sw") {
         memList.setValueAt((currentInstruction.rt + currentInstruction.immediate),currentInstruction.rs);
